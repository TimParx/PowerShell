...
$destination_folder = "\\btsevlev02.se.ad.banctec.com\Leverans\OCFileDelivery\ManualResend\ToResend\output\$dir"
$destination_folder2 = "\\btsevlev02.se.ad.banctec.com\Leverans\OCFileDelivery\ManualResend\ToResend\output\$dir\$dir2"
...
$user = $env:UserName
$LogFile1 = '\\btsevlev02.se.ad.banctec.com\Leverans\OCFileDelivery\ManualResend\ToResend\logs\' + $found + $LogTime + ".log"
$LogFile2 = '\\btsevlev02.se.ad.banctec.com\Leverans\OCFileDelivery\ManualResend\ToResend\logs\' + $missing + $LogTime + ".log"
$LogFile3 = '\\btsevlev02.se.ad.banctec.com\Leverans\OCFileDelivery\ManualResend\ToResend\logs\' + $coppied + $LogTime + '_' + $user + ".log"
$LogFile4 = '\\btsevlev02.se.ad.banctec.com\Leverans\OCFileDelivery\ManualResend\ToResend\logs\' + $checked + $LogTime + '_' + $user + ".log"
$LogFile5 = '\\btsevlev02.se.ad.banctec.com\Leverans\OCFileDelivery\ManualResend\ToResend\logs\' + $corrupted + $LogTime + '_' + $user + ".log"




# Get-Content \\btsevlev02.se.ad.banctec.com\Leverans\OCFileDelivery\ManualResend\ToResend\input.txt | Where-Object {$_.trim() -ne "" } | Out-File proper.txt

$input_file = Get-Content \\btsevlev02.se.ad.banctec.com\Leverans\OCFileDelivery\ManualResend\ToResend\input.txt | Where-Object { $_.trim() -ne "" }
...
IF ($input_file_proper.count -gt 0) {
    ...
    if ([IO.Directory]::Exists($destination_folder)) {
        ...
        New-Item -ItemType directory -Path \\btsevlev02.se.ad.banctec.com\Leverans\OCFileDelivery\ManualResend\ToResend\output\$dir | Out-Null
    ...
    New-Item -ItemType directory -Path \\btsevlev02.se.ad.banctec.com\Leverans\OCFileDelivery\ManualResend\ToResend\output\$dir\$dir2 | Out-Null # cza dodac jeszcze usuwanie folderu jak nic nie znajdzie
    ...
    if (Test-Path -Path $LogFile1) {
        ...
        #input variables
        $zipInputFolder = "\\btsevlev02.se.ad.banctec.com\Leverans\OCFileDelivery\ManualResend\ToResend\output\$dir\$dir2"
        $zipOutPutFolder = "\\btsevlev02.se.ad.banctec.com\Leverans\OCFileDelivery\ManualResend\ToResend\temp\$dir\$dir2"
        $corrupted_array = @()
        $corrupted_output = "\\btsevlev02.se.ad.banctec.com\Leverans\OCFileDelivery\ManualResend\ToResend\corrupted\$dir\$dir2"
        $proper_array = @()
        #start
        ...
        foreach ($catalog in $catalog_list) {n
            ...
            foreach ($plik in $lista_pliki) {
                ...
                #Email documents /ERIN Rejected
                elseif ($plik.name.length -eq '83') {
                
                    if ($plik.Name -match '^ERIN_[0-9]{14}_[a-z0-9]{18}_[A-Z0-9]{32}siP[0-9]{1}.ack.xml$') {
                        Write-Host $plik.Name ...FILE OK
                        $list_batches_ok += $catalog.Name
                    }
                    else {
    ...
    if (Test-Path -Path $LogFile4) {
        ...
        If ($question -eq 'y') {
            ...
            $search_folder2 = "\\btsevlev02.se.ad.banctec.com\Leverans\OCFileDelivery\ManualResend\ToResend\output\$dir\$dir2"
